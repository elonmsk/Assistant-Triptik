// app/api/update-disability/route.ts
...
export async function POST(req: Request) {
  const { numero, disability } = await req.json()

  if (!numero || !disability) {
    return NextResponse.json({ error: "Champs manquants" }, { status: 400 })
  }

  const { error } = await supabase
    .from("info")
    .update({ "situation de handicap": disability })
    .eq("numero", numero)
  ...
  return NextResponse.json({ message: "Situation de handicap enregistrée avec succès" })
}
